<#
  ...
  Does everything: detects the Pi/broker IP, patches the firmware, builds BOTH
  firmwares (node + jammer), flashes node->COM6 and jammer->COM7, and opens the
  live dashboard.

  Usage:
    .\run.ps1                       # full: detect IP, build, flash both, dashboard
    .\run.ps1 -Node COM6 -Jammer COM7
    .\run.ps1 -SkipBuild            # skip build+flash, just open the dashboard
